...
from nokku.lottery.kerala.astrology_interpretation import INTERPRETATION_METHOD
...
def test_living_loop_derives_and_preserves_candidate_astrology_without_policy_effect(tmp_path):
    ...
    assert result.natal_astrology_context.lunar_nodes is not None
    ...
    natal = signals["natal_astrology_context"]
    assert natal["status"] == "success"
    assert natal["moon"] is not None
    assert natal["jupiter"] is not None
    assert natal["lunar_nodes"] is not None
    assert natal["lunar_nodes"]["convention"] == "osculating_true_ecliptic_equinox_of_date"

    assert len(signals["candidate_astrology"]) == 1
    stored = signals["candidate_astrology"][0]
    assert stored["status"] == "success"
    assert stored["method"] == INTERPRETATION_METHOD
    assert stored["target_at"] == "2026-09-12T15:00:00+05:30"
    assert stored["transit_moon_to_natal_ketu_separation_deg"] is not None
