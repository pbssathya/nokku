...
def event_code(lottery_name: str) -> str:
    """Return the published draw code (for validation only, not source identity)."""
...
def discover_known_draws(memory: Memory):
    """Recall Kerala collect experiences without assuming a numeric source namespace."""
    flow = Flow()
    ...
    # Memory receipts are returned in stored order. Later corrected/repeated
    # experiences therefore replace earlier ones for the same explicit source.
...
def main() -> int:
    print(f"\n=== NOKKU LIVING HABITAT — SOURCE-AWARE {YEAR} BACKFILL ===")
    ...
    assert callable(discover_legacy), (
        "Collector does not expose the validated Kerala legacy-year source capability. "
        "Use the Collector branch/merge that contains legacy_sources_for_year()."
    )
    ...
    existing_modern = [item for item in existing_target if item[0].isdigit()]
    ...
    print(f"   existing {YEAR} modern records:", len(existing_modern))
    ...
    assert len(legacy_sources) == len(set(legacy_sources)), "Legacy source discovery returned duplicates."
        ...
        if known_item:
            ...
            if draw_date_text and draw_date_text != "Unknown":
                ...
                if draw_date and draw_date.year == YEAR:
                    skipped_known += 1
                    print(f"   [{index:03d}/{len(legacy_sources):03d}] {source} | already known")
                    continue
    ...
    modern_after = [item for item in target_after if item[0].isdigit()]
    ...
    assert len(legacy_after) >= len(legacy_sources)

    # Validate the observed transition evidence rather than inventing a generic
    # deduplication mechanism. A duplicate event would be the same draw code on the
    # same held date appearing through more than one explicit source address.
    ...
    assert not duplicate_events, (
        "Exact event duplication appeared across source namespaces: "
        f"{duplicate_events}"
    )

    # Calendar accounting is about events, not source-address arithmetic.
    ...
    bumper_draws = [
        item for item in target_after if "BUMPER" in item[2].upper()
    ]
    month_counts = Counter(draw_date.strftime("%Y-%m") for _, draw_date, _ in target_after)

    modern_numbers = sorted(int(source) for source, _, _ in modern_after)
    legacy_numbers = sorted(
        int(source.split(":", 1)[1]) for source in legacy_sources
    )
    ...
    if modern_numbers:
        print(
            "   modern drawserial address range:",
            min(modern_numbers),
            "→",
            max(modern_numbers),
        )
    ...
    print("\n✅ PASSED — source-aware 2020 backfill")
    return 0
